...
from custom_operator.utilities.http_utils import HttpUtils
from custom_operator.utilities.operator_util import *

job_name = 'ExecuteAthenaToRedshiftQueryOperator JOB'


class AthenaToRedshiftOperator(BaseOperator):
    ui_color = "#BBD915"
    ...
    def __create_table(self, stage_table_name, final_table_name):
        """
        ...
        :return: none
        """
        logging.getLogger(job_name).info("Create table running for table - {}".format(final_table_name))
        final_table_query = constant_queries.create_table_query.format(final_table_name,
                                                                       constant_queries.select_all_query.format(
                                                                           stage_table_name + ' limit 0'))
        grant_access = constant_queries.grant_access_group_query.format(final_table_name,
                                                                        constant_values.redshift_report_group_name)
        self.redshift_conn.execute_query(final_table_query + ';' + grant_access + ';', job_name)
    ...
    def __check_and_update_main_table(self, stage_table_name, final_table_name):
        """
        ...
        column_details_to_add, column_details_to_delete, column_details_to_alter = get_column_diff(new_columns_details,
                                                                                                   old_columns_details)
    ...
    def __read_athena_load_to_redshift_stage(self, query, stage_table_name):
        """
        Calls Datatitan engine to fetch data from athena and move to redshift stage
        :param query: Query to load data from source
        :param stage_table_name: Name of stage table to insert into
        :return: none
        """
        logging.getLogger(job_name).info(
            " Submitting Job request to titan for Athena to Redshift for table {}".format(stage_table_name))
        http_utils = HttpUtils()
        titan_resource_tier = Variable.get(constant_values.titan_resource_tier)
        titan_engine_name = Variable.get(constant_values.titan_engine_name)
        titan_job_config_retry = int(Variable.get(constant_values.titan_job_config_retry))
        titan_job_config_cluster_family = Variable.get(constant_values.titan_job_config_cluster_family)
        request_body = {
            'jobType': 'BATCH',
            'resourceTier': titan_resource_tier,
            'query': query,
            'source': [],
            'destination': [{
                'type': 'REDSHIFT',
                'config': {
                    ...
                    'engineName': titan_engine_name,
                    'mode': 'OVERWRITE'}
            }], 'jobConfigs': {
                'retries': titan_job_config_retry,
                'delay': 5,
                'useClusterFamily': titan_job_config_cluster_family
            }
        }
        logging.getLogger(job_name).info(
            " Request Body for Titan {}".format(request_body))
        titan_url = Variable.get(constant_values.titan_url)
        job_submit_url = titan_url + 'job/submit/'
        job_status_url = titan_url + 'job/status/request-id='
        titan_auth = Variable.get(constant_values.titan_auth)
        titan_header = constant_values.titan_headers
        titan_header['Authorization'] = titan_auth
        response, status = http_utils.post(request_body, job_name, job_submit_url, titan_header)
        logging.getLogger(job_name).info(
            " Response From Titan {}".format(response))
        if status:
            requestid = response.get('jobDetails').get('requestId')
            job_status = response.get('jobDetails').get('status')
            logging.getLogger(job_name).info(
                " Titan Job Status {}".format(job_status))
            while job_status == 'SUBMITTING' or job_status == 'RUNNING':
                job_status_reponse = http_utils.get(job_name, job_status_url + str(requestid), titan_header)
                job_status = job_status_reponse.get('status')
                if job_status == 'FAILED':
                    error_message = job_status_reponse.get('errorMessage')
                    logging.getLogger(job_name).error(error_message)
                    raise Exception(error_message)
                time.sleep(30)
            logging.getLogger(job_name).info(" Job request completed by Titan for table {}".format(stage_table_name))
        return

    def dedup_then_insert(self, dedup_query, insert_query, full_table_name):
        """
        Function to drop and insert into table
        :param dedup_query: Query to delete from main table new table
        :param insert_query: Query to get data from_temp table;
        :param full_table_name: Name of table to insert data into
        :return: none
        """
        logging.getLogger(job_name).info("dedup and insert running for table - {}".format(full_table_name))
        query = dedup_query + ";" + constant_queries.insert_into_query.format(full_table_name, insert_query) + ";"
        self.redshift_conn.execute_query(query, job_name)
        logging.getLogger(job_name).info("dedup and insert completed for table - {}".format(full_table_name))

    @maintain_node_run
    def __incremental_load(self, stage_table_name, final_table_name, id_column_name,
        ...
        self.__read_athena_load_to_redshift_stage(query, stage_table_name)
        if not is_table_created:
            self.__create_table(stage_table_name, final_table_name)
        if not self.__check_table_count(stage_table_name):
            return
        self.__check_and_update_main_table(stage_table_name, final_table_name)
        dedup_query = constant_queries.delete_rows_query_with_where.format(final_table_name, id_column_name,
                                                                           stage_table_name)
        logging.getLogger(job_name).info("Stage table updated with latest data - {}".format(final_table_name))
        insert_query = constant_queries.select_all_query.format(stage_table_name)
        self.dedup_then_insert(dedup_query, insert_query, final_table_name)
        logging.getLogger(job_name).info("Incremental load completed for table - {}".format(final_table_name))

    @maintain_node_run
    def __truncate_load(self, stage_table_name, main_table_name, is_table_created, node_record,
        ...
        :return: none
        """
        logging.getLogger(job_name).info("Truncate load started for table - {}".format(main_table_name))
        self.__read_athena_load_to_redshift_stage(query, stage_table_name)
        if not is_table_created:
            self.__create_table(stage_table_name, main_table_name)
        self.__check_and_update_main_table(stage_table_name, main_table_name)
        if not self.__check_table_count(stage_table_name):
            return
    ...
    def execute(self, context):
        """
        Executes logic of ExecuteAthenaToRedshiftQueryOperator operator
        :param context: Context of airflow
        :return: none
        """
        logging.getLogger(job_name).info(
            "Execution started for Athena to Redshift task - {}".format(self.task_to_run_id))
        ...
        if run_type == 'TRUNCATELOAD':
            self.__truncate_load(stage_table_name,
                                 main_table_name, len(destination_table_info) > 0,
            ...
            self.__incremental_load(stage_table_name,
                                    main_table_name, destination['idColumns'], len(destination_table_info) > 0,
                                    node_record=node_record, dag_run_id=context['dag_run'].run_id,
                                    mysql=mysql, query=query)
